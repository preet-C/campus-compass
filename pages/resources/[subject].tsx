...
import { 
  FileText, Download, Search, Plus, X, Trash2, 
  ChevronRight, BookOpen, FileQuestion, BookMarked,
  Upload, Loader2
} from 'lucide-react'
...
export default function SubjectDetail() {
  ...
  function filterResourcesByType(type: ResourceType): Resource[] {
    const typeMap: { [key in ResourceType]: string[] } = {
      ...
      'Question Papers': ['question paper', 'question papers', 'pyq', 'pyqs', 'qp', 'previous year', 'previous years'],
      'Reference': ['reference', 'reference materials', 'reference material', 'ref', 'references']
    }
    ...
    return resources.filter(r => {
      ...
      const matchesType = searchTerms.some(term => 
        resourceType === term || resourceType.includes(term) || term.includes(resourceType)
      )
  ...
  async function handleAddResource(e: React.FormEvent) {
    e.preventDefault()
    if (!newResourceFile || !newResourceTitle || !decodedSubject || !year) return
  ...
  const breadcrumbs = [
    ...
    { label: `Year ${year}`, href: null },
    ...(parseInt(year as string) === 1 && cycle ? [{ label: cycle, href: null }] : []),
    ...(branch ? [{ label: branch, href: null }] : []),
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 pb-20">
      ...
      <div className="max-w-6xl mx-auto px-4 pt-6">
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            <h1 className="text-3xl font-black text-gray-900 mb-1">{decodedSubject}</h1>
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {(['Notes', 'Question Papers', 'Reference'] as ResourceType[]).map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-6 py-3 rounded-xl font-bold text-sm whitespace-nowrap transition flex items-center gap-2 ${
                activeTab === tab
            ...
            >
              ...
              {tab === 'Question Papers' && <FileQuestion size={18} />}
              {tab === 'Reference' && <BookMarked size={18} />}
              {tab}
            </button>
          ))}
        </div>
          ...
          <>
            ...
            <div className="md:hidden space-y-6">
                    ...
                    <div className="space-y-3">
                      {notes.map(resource => (
                        <ResourceCard
                          ...
                          resource={resource}
                          isAdmin={isAdmin}
                          onDelete={() => handleDeleteResource(resource.id, resource.link)}
                        />
                      ))}
                    </div>
                  ) : (
                    <EmptyState message="No study notes available yet" />
                  )}
                </div>
              )}
              {activeTab === 'Question Papers' && (
                <div>
                  <h2 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4 flex items-center gap-2">
                    <FileQuestion size={16} /> Question Papers (PYQs)
                  </h2>
              ...
              {activeTab === 'Reference' && (
                <div>
                  <h2 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4 flex items-center gap-2">
                    <BookMarked size={16} /> Reference Materials
                  </h2>
                  {reference.length > 0 ? (
                    <div className="space-y-3">
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-black text-gray-900">Add Resource</h2>
                <button
              ...
              <form onSubmit={handleAddResource} className="space-y-4">
                ...
                <div className="bg-gray-50 p-3 rounded-lg text-sm">
                  ...
                  {cycle && <p><span className="font-bold">Cycle:</span> {cycle}</p>}
                  {branch && <p><span className="font-bold">Branch:</span> {branch}</p>}
                  <p><span className="font-bold">Subject:</span> {decodedSubject}</p>
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="Question Papers">Question Papers (PYQs)</option>
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                    ...
                    >
                      ...
                      <span className="text-sm text-gray-600">
                        {newResourceFile ? newResourceFile.name : 'Click to upload PDF'}
                      </span>
                    </label>
                  </div>
                </div>
...
function ResourceCard({ resource, isAdmin, onDelete }: { resource: Resource; isAdmin: boolean; onDelete: () => void }) {
  return (
    <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition group relative">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-bold text-gray-900 text-sm mb-1 line-clamp-2">{resource.title}</h3>
          <a
            href={resource.link}
            target="_blank"
            rel="noreferrer"
